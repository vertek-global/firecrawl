...
describe("E2E Tests for API Routes with No Authentication", () => {
    ...
    describe("GET /v0/crawl/status/:jobId", () => {
        ...
        it("should return Job not found for invalid job ID", async () => {
            const response = await (0, supertest_1.default)(TEST_URL).get("/v0/crawl/status/invalidJobId");
        ...
        it("should return a successful response for a valid crawl job", async () => {
            ...
            const response = await (0, supertest_1.default)(TEST_URL).get(`/v0/crawl/status/${crawlResponse.body.jobId}`);
            expect(response.statusCode).toBe(200);
            expect(response.body).toHaveProperty("status");
            expect(response.body.status).toBe("active");
            // wait for 30 seconds
            await new Promise((r) => setTimeout(r, 30000));
            const completedResponse = await (0, supertest_1.default)(TEST_URL).get(`/v0/crawl/status/${crawlResponse.body.jobId}`);
