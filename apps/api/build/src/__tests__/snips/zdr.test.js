...
if (process.env.TEST_SUITE_SELF_HOSTED) {
    ...
    async function getServerLogs() {
        if (!process.env.GITHUB_ACTIONS) {
            try {
                await (0, promises_1.stat)("api.log");
            }
            catch (e) {
    ...
    async function getWorkerLogs() {
        ...
        const logs = await (0, promises_1.readFile)("worker.log", "utf8");
        return logs.split("\n").filter(x => x.trim().length > 0 && !logIgnoreList.some(y => x.includes(y)));
    }
    describe("Zero Data Retention", () => {
        describe.each(["Team-scoped", "Request-scoped"])("%s", (scope) => {
            it("should clean up a scrape immediately", async () => {
                let identity = await (0, lib_1.idmux)({
                ...
                const scrape1 = await (0, lib_1.scrape)({
                    url: "https://firecrawl.dev/?test=" + testId,
                    zeroDataRetention: scope === "Request-scoped" ? true : undefined,
                }, identity);
                const gcsJob = await (0, gcs_jobs_1.getJobFromGCS)(scrape1.metadata.scrapeId);
                expect(gcsJob).toBeNull();
                const { data, error } = await supabase_1.supabase_service.from("firecrawl_jobs")
                    .select("*")
                ...
                if (scope === "Request-scoped") {
                    const status = await (0, lib_1.scrapeStatusRaw)(scrape1.metadata.scrapeId, identity);
            ...
            it("should clean up a crawl", async () => {
                ...
                const crawl1 = await (0, lib_1.crawl)({
                ...
                const { data: jobs, error: jobsError } = await supabase_1.supabase_service.from("firecrawl_jobs")
                    .select("*")
                ...
                for (const job of jobs ?? []) {
                    ...
                    if (job.success) {
                        const gcsJob = await (0, gcs_jobs_1.getJobFromGCS)(job.job_id);
                        expect(gcsJob).not.toBeNull(); // clean up happens async on a worker after expiry
                    }
                }
                await (0, lib_1.zdrcleaner)(identity.teamId);
                for (const job of jobs ?? []) {
                    const gcsJob = await (0, gcs_jobs_1.getJobFromGCS)(job.job_id);
                    expect(gcsJob).toBeNull();
                    if (scope === "Request-scoped") {
                        const status = await (0, lib_1.scrapeStatusRaw)(job.job_id, identity);
                        expect(status.statusCode).toBe(404);
                    }
                }
            }, 600000);
            it("should clean up a batch scrape", async () => {
                ...
                const crawl1 = await (0, lib_1.batchScrape)({
...
//# sourceMappingURL=zdr.test.js.map