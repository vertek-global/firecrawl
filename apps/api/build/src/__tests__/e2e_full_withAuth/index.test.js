...
describe("E2E Tests for API Routes", () => {
    ...
    describe("GET /", () => {
        it.concurrent("should return Hello, world! message", async () => {
            const response = await (0, supertest_1.default)(TEST_URL).get("/");
    ...
    describe("GET /test", () => {
        it.concurrent("should return Hello, world! message", async () => {
            const response = await (0, supertest_1.default)(TEST_URL).get("/test");
            expect(response.statusCode).toBe(200);
    ...
    describe("POST /v0/scrape", () => {
        it.concurrent("should require authorization", async () => {
            const response = await (0, supertest_1.default)(TEST_URL).post("/v0/scrape");
            expect(response.statusCode).toBe(401);
        });
        it.concurrent("should return an error response with an invalid API key", async () => {
            const response = await (0, supertest_1.default)(TEST_URL)
                .post("/v0/scrape")
        ...
        it.concurrent("should return an error for a blocklisted URL", async () => {
            ...
            expect(response.body.error).toContain(strings_1.BLOCKLISTED_URL_MESSAGE);
        });
        ...
        it.concurrent("should return a successful response with a valid API key with removeTags option", async () => {
            const responseWithoutRemoveTags = await (0, supertest_1.default)(TEST_URL)
                .post("/v0/scrape")
                .set("Authorization", `Bearer ${process.env.TEST_API_KEY}`)
                .set("Content-Type", "application/json")
    ...
    describe("POST /v0/crawl", () => {
        it.concurrent("should require authorization", async () => {
            const response = await (0, supertest_1.default)(TEST_URL).post("/v0/crawl");
            expect(response.statusCode).toBe(401);
        });
        it.concurrent("should return an error response with an invalid API key", async () => {
            const response = await (0, supertest_1.default)(TEST_URL)
        ...
        it.concurrent("should prevent duplicate requests using the same idempotency key", async () => {
            ...
            const firstResponse = await (0, supertest_1.default)(TEST_URL)
                .post("/v0/crawl")
                .set("Authorization", `Bearer ${process.env.TEST_API_KEY}`)
                .set("Content-Type", "application/json")
            ...
            const secondResponse = await (0, supertest_1.default)(TEST_URL)
                .post("/v0/crawl")
                .set("Authorization", `Bearer ${process.env.TEST_API_KEY}`)
                .set("Content-Type", "application/json")
                .set("x-idempotency-key", uniqueIdempotencyKey)
        ...
        it.concurrent("should return a successful response with a valid API key and valid includes option", async () => {
            const crawlResponse = await (0, supertest_1.default)(TEST_URL)
                .post("/v0/crawl")
                .set("Authorization", `Bearer ${process.env.TEST_API_KEY}`)
                .set("Content-Type", "application/json")
                .send({
            ...
            while (!isFinished) {
                response = await (0, supertest_1.default)(TEST_URL)
            ...
            const urls = completedResponse.body.data.map((item) => item.metadata?.sourceURL);
        ...
        it.concurrent("should return a successful response with max depth option for a valid crawl job", async () => {
            ...
            while (!isCompleted) {
                const statusCheckResponse = await (0, supertest_1.default)(TEST_URL)
                    .get(`/v0/crawl/status/${crawlResponse.body.jobId}`)
                    .set("Authorization", `Bearer ${process.env.TEST_API_KEY}`);
            ...
            const completedResponse = await (0, supertest_1.default)(TEST_URL)
                .get(`/v0/crawl/status/${crawlResponse.body.jobId}`)
                .set("Authorization", `Bearer ${process.env.TEST_API_KEY}`);
            expect(completedResponse.statusCode).toBe(200);
            expect(completedResponse.body).toHaveProperty("status");
        ...
        it.concurrent("should return a successful response with relative max depth option for a valid crawl job with maxDepths equals to zero", async () => {
            ...
            const completedResponse = await (0, supertest_1.default)(TEST_URL)
                .get(`/v0/crawl/status/${crawlResponse.body.jobId}`)
                .set("Authorization", `Bearer ${process.env.TEST_API_KEY}`);
            const testurls = completedResponse.body.data.map((item) => item.metadata?.sourceURL);
        ...
        it.concurrent("should crawl external content links when allowed", async () => {
            const crawlInitResponse = await (0, supertest_1.default)(TEST_URL)
                .post("/v0/crawl")
                .set("Authorization", `Bearer ${process.env.TEST_API_KEY}`)
                .set("Content-Type", "application/json")
                .send({
            ...
            while (crawlStatus !== "completed") {
                const statusResponse = await (0, supertest_1.default)(TEST_URL)
            ...
            expect(crawlData).toEqual(expect.arrayContaining([
                expect.objectContaining({
                    url: expect.stringContaining("https://firecrawl.dev/?ref=mendable+banner"),
                }),
                expect.objectContaining({
    ...
    describe("POST /v0/crawlWebsitePreview", () => {
        it.concurrent("should require authorization", async () => {
            const response = await (0, supertest_1.default)(TEST_URL).post("/v0/crawlWebsitePreview");
            expect(response.statusCode).toBe(401);
        });
        it.concurrent("should return an error response with an invalid API key", async () => {
            const response = await (0, supertest_1.default)(TEST_URL)
    ...
    describe("POST /v0/search", () => {
        it.concurrent("should require authorization", async () => {
            const response = await (0, supertest_1.default)(TEST_URL).post("/v0/search");
            expect(response.statusCode).toBe(401);
        });
        it.concurrent("should return an error response with an invalid API key", async () => {
            const response = await (0, supertest_1.default)(TEST_URL)
    ...
    describe("GET /v0/crawl/status/:jobId", () => {
        it.concurrent("should require authorization", async () => {
            const response = await (0, supertest_1.default)(TEST_URL).get("/v0/crawl/status/123");
            expect(response.statusCode).toBe(401);
        });
        it.concurrent("should return an error response with an invalid API key", async () => {
            const response = await (0, supertest_1.default)(TEST_URL)
        ...
        it.concurrent("should return a successful crawl status response for a valid crawl job", async () => {
            ...
            expect(completedResponse.body.data[0].metadata.pageStatusCode).toBe(200);
            expect(completedResponse.body.data[0].metadata.pageError).toBeUndefined();
            const childrenLinks = completedResponse.body.data.filter((doc) => doc.metadata &&
    ...
    it.concurrent("If someone cancels a crawl job, it should turn into failed status", async () => {
        ...
        const responseCancel = await (0, supertest_1.default)(TEST_URL)
        ...
        it.concurrent("should extract data using LLM extraction mode with RawHtml", async () => {
            const response = await (0, supertest_1.default)(TEST_URL)
                ...
                extractorOptions: {
                    ...
                    extractionPrompt: "Based on the information on the page, what are the primary and secondary CTA buttons?",
                    extractionSchema: {
                        type: "object",
                        properties: {
                            primary_cta: {
    ...
    describe("POST /v0/crawl with fast mode", () => {
        it.concurrent("should complete the crawl under 20 seconds", async () => {
            ...
            let isFinished = false;
            while (!isFinished) {
                statusResponse = await (0, supertest_1.default)(TEST_URL)
    ...
    describe("GET /is-production", () => {
        it.concurrent("should return the production status", async () => {
            const response = await (0, supertest_1.default)(TEST_URL).get("/is-production");
            expect(response.statusCode).toBe(200);
