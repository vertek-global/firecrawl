...
describe("E2E Tests for Extract API Routes", () => {
    it.concurrent("should return authors of blog posts on firecrawl.dev", async () => {
        ...
        for (const author of response.body.data?.authors) {
            if (author.includes("Caleb Peffer"))
                gotItRight++;
            if (author.includes("Gergő Móricz"))
                gotItRight++;
            if (author.includes("Eric Ciarla"))
                gotItRight++;
            if (author.includes("Nicolas Camara"))
                gotItRight++;
            if (author.includes("Jon"))
                gotItRight++;
            if (author.includes("Wendong"))
                gotItRight++;
        }
    ...
    it.concurrent("should return founders of firecrawl.dev (allowExternalLinks = true)", async () => {
        ...
        for (const founder of response.body.data?.founders) {
            if (founder.includes("Caleb"))
                gotItRight++;
            if (founder.includes("Eric"))
                gotItRight++;
            if (founder.includes("Nicolas"))
                gotItRight++;
            if (founder.includes("nick"))
                gotItRight++;
            if (founder.includes("eric"))
                gotItRight++;
            if (founder.includes("jon-noronha"))
                gotItRight++;
        }
    ...
    it.concurrent("should return hiring opportunities on firecrawl.dev (allowExternalLinks = true)", async () => {
        ...
        for (const hiring of response.body.data?.items) {
            if (hiring.includes("Firecrawl Example Creator"))
                gotItRight++;
            if (hiring.includes("Senior Frontend Engineer"))
                gotItRight++;
            if (hiring.includes("Technical Chief of Staff"))
                gotItRight++;
            if (hiring.includes("Founding Web Automation Engineer"))
                gotItRight++;
            if (hiring.includes("Founding Fullstack Engineer"))
                gotItRight++;
        }
    ...
    it.concurrent("should return mintlify api components", async () => {
        ...
        for (const component of response.body.data?.items) {
            ...
            if (component.component.toLowerCase().includes("expandable"))
                gotItRight++;
            if (component.component.toLowerCase().includes("sticky"))
                gotItRight++;
    ...
    it.concurrent("should extract information with scrapeOptions.waitFor", async () => {
        const response = await (0, supertest_1.default)(TEST_URL)
            .post("/v1/extract")
            .set("Authorization", `Bearer ${process.env.TEST_API_KEY}`)
            .set("Content-Type", "application/json")
