...
function cleanOfNull(x) {
    if (Array.isArray(x)) {
        return x.map((x) => cleanOfNull(x));
    }
    else if (typeof x === "object" && x !== null) {
        return Object.fromEntries(Object.entries(x).map(([k, v]) => [k, cleanOfNull(v)]));
    }
...
async function logJob(job, force = false, bypassLogging = false) {
    ...
    try {
        ...
        if (process.env.GCS_BUCKET_NAME) {
            await (0, gcs_jobs_1.saveJobToGCS)(job);
        }
        if (bypassLogging) {
            return;
        }
            ...
            while (i++ <= 10) {
                try {
                    const { error } = await supabase_1.supabase_service
                        ...
                        .insert([jobColumn]);
                    if (error) {
                        logger.error("Failed to log job due to Supabase error -- trying again", { error });
                    ...
                    logger.error("Failed to log job due to thrown error -- trying again", { error });
                    await new Promise((resolve) => setTimeout(() => resolve(), 75));
                }
            }
            if (done) {
