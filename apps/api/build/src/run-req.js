...
async function sendCrawl(result) {
    const idempotencyKey = (0, uuid_1.v4)();
    const url = result.start_url;
    try {
        const response = await axios_1.default.post("https://staging-firecrawl-scraper-js.fly.dev/v0/crawl", {
            url: url,
            crawlerOptions: {
                limit: 75,
            },
...
async function getContent(result) {
    let attempts = 0;
    while (attempts < 120) {
        // Reduce the number of attempts to speed up
        try {
            const response = await axios_1.default.get(`https://staging-firecrawl-scraper-js.fly.dev/v0/crawl/status/${result.job_id}`, {
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer `,
                },
    ...
    const processSingleResult = async (result) => {
        ...
        if (jobId) {
            ...
            try {
                ...
                await fs_1.promises.writeFile("results_with_job_id_4000_6000.json", JSON.stringify(resultWithJobId, null, 4));
            }
            catch (error) {
...
async function getStartUrls() {
    try {
        const data = await fs_1.promises.readFile("starturls.json", "utf-8");
...
async function main() {
    const results = (await getStartUrls()).slice(3999, 6000);
...
main();
//# sourceMappingURL=run-req.js.map