...
exports.decryptedBlocklist = decryptedBlocklist;
...
function encryptAES(plaintext, key) {
    const cipher = crypto_1.default.createCipheriv(algorithm, key, null);
...
function decryptAES(ciphertext, key) {
    const decipher = crypto_1.default.createDecipheriv(algorithm, key, null);
...
function isUrlBlocked(url, flags) {
    ...
    const decryptedUrl = blockedlist.find((decrypted) => lowerCaseUrl === decrypted) ||
        lowerCaseUrl;
    // If the URL is empty or invalid, return false
    let parsedUrl;
    try {
        parsedUrl = (0, tldts_1.parse)(decryptedUrl);
    }
    catch {
    ...
    if (allowedKeywords.some((keyword) => lowerCaseUrl.includes(keyword.toLowerCase()))) {
        return false;
    }
    // Block exact matches
    ...
    if (publicSuffix &&
        blockedlist.some((blocked) => blocked.startsWith(baseDomain + ".") && blocked !== domain)) {
        return true;
    }
    return false;
}
