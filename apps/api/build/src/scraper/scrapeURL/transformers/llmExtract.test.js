...
const llmExtract_2 = require("./llmExtract");
...
describe("removeDefaultProperty", () => {
    it("should remove the default property from a simple object", () => {
        ...
        expect((0, llmExtract_1.removeDefaultProperty)(input)).toEqual(expectedOutput);
    });
    ...
    it("should handle null and non-object inputs", () => {
        expect((0, llmExtract_1.removeDefaultProperty)(null)).toBeNull();
        expect((0, llmExtract_1.removeDefaultProperty)("string")).toBe("string");
        expect((0, llmExtract_1.removeDefaultProperty)(123)).toBe(123);
    });
});
describe("trimToTokenLimit", () => {
    ...
    beforeEach(() => {
        jest.clearAllMocks();
        tiktoken_1.encoding_for_model.mockReturnValue(mockEncoder);
    });
    it("should return original text if within token limit", () => {
        const text = "This is a test text";
        mockEncode.mockReturnValue(new Array(5)); // Simulate 5 tokens
        const result = (0, llmExtract_2.trimToTokenLimit)(text, 10, "gpt-4o");
    ...
    it("should append previous warning if provided", () => {
        ...
        const result = (0, llmExtract_2.trimToTokenLimit)(text, 10, "gpt-4o", previousWarning);
        expect(result.warning).toContain("automatically trimmed");
    ...
    it("should handle large token limits (128k)", () => {
        ...
        const result = (0, llmExtract_2.trimToTokenLimit)(text, 128000, "gpt-4o");
        expect(result.text.length).toBeLessThan(text.length);
    ...
    it("should handle large token limits (512k) with 32k context window", () => {
        ...
        const result = (0, llmExtract_2.trimToTokenLimit)(text, 32000, "gpt-4o");
        expect(result.text.length).toBeLessThan(text.length);
    ...
    it("should append new warning to previous warning", () => {
        ...
        const result = (0, llmExtract_2.trimToTokenLimit)(text, 50, "gpt-4o", previousWarning);
        expect(result.warning).toContain("automatically trimmed");
        expect(result.warning).toContain(previousWarning);
        expect(mockFree).toHaveBeenCalled();
    });
    it("should handle encoder initialization failure gracefully", () => {
        ...
        tiktoken_1.encoding_for_model.mockImplementationOnce(() => {
    ...
    it("should handle very small token limits", () => {
        ...
        const result = (0, llmExtract_2.trimToTokenLimit)(text, 3, "gpt-4o");
        expect(result.text.length).toBeLessThan(text.length);
    ...
    it("should handle unicode characters", () => {
        ...
        const result = (0, llmExtract_2.trimToTokenLimit)(text, 4, "gpt-4o");
        expect(result.text.length).toBeLessThan(text.length);
    ...
    it("should handle multiple trimming iterations", () => {
        ...
        mockEncode
            ...
            .mockReturnValueOnce(new Array(100))
            .mockReturnValueOnce(new Array(50));
        const result = (0, llmExtract_2.trimToTokenLimit)(text, 50, "gpt-4o");
        expect(result.text.length).toBeLessThan(text.length);
...
//# sourceMappingURL=llmExtract.test.js.map