...
async function extractDataWithSchema(content, meta) {
    try {
...
function compareExtractedData(previousData, currentData) {
    const result = {};
...
async function deriveDiff(meta, document) {
    if (meta.options.formats.includes("changeTracking")) {
        ...
        const res = await supabase_1.supabase_service
        ...
        const job = data?.o_job_id ? await (0, crawl_status_1.getJob)(data.o_job_id) : null;
        if (data && job && job?.returnvalue) {
            const previousMarkdown = job.returnvalue.markdown;
            const currentMarkdown = document.markdown;
            const transformer = (x) => [...x.replace(/\s+/g, "").replace(/\[iframe\]\(.+?\)/g, "")].sort().join("");
            ...
            if (meta.options.changeTrackingOptions?.modes?.includes("git-diff") && changeStatus === "changed") {
                const diffText = (0, git_diff_1.default)(previousMarkdown, currentMarkdown, {
                    color: false,
                    wordDiff: false
                });
                meta.logger.debug("Diff text", { diffText });
                if (diffText) {
                    const diffStructured = (0, parse_diff_1.default)(diffText);
