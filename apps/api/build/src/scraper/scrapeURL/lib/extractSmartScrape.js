...
exports.makeSchemaNullable = makeSchemaNullable;
exports.prepareSmartScrapeSchema = prepareSmartScrapeSchema;
...
const commonSmartScrapeProperties = {
    shouldUseSmartscrape: {
        type: "boolean",
        description: "Set to `true` if any of the extractedData is null and you think you can find the information by performing user-like interactions (e.g., clicking buttons/accordions to reveal hidden text, login, inputs, pagination etc.). SmartScrape can perform these actions to access the data.",
    },
...
const commonReasoningPromptProperties = {
    smartscrape_reasoning: {
        ...
        description: "Reasoning for why a SmartScrape is needed. Explain which data is missing or requires interaction.",
    },
    smartscrape_prompt: {
        type: ["string", "null"],
...
const multiSmartScrapeWrapperSchemaDefinition = {
    ...
    properties: {
        ...
        smartScrapePages: {
            type: "array",
            description: "Make an entry for each page we want to run smart scrape on, no matter how many actions it should be one entry per page.",
            items: {
                type: "object",
                properties: {
                    page_index: {
    ...
    for (const key in schema) {
        ...
        if (key === "type") {
            ...
            if (typeof currentType === "string") {
                ...
                if (currentType.includes("object"))
                    isObject = true;
 ...
 */
function prepareSmartScrapeSchema(originalSchema, logger, isSingleUrl) {
...
async function extractData({ extractOptions, urls, useAgent, extractId, sessionId, scrapeId, }) {
    let schema = extractOptions.options.schema;
    const logger = extractOptions.logger;
    const isSingleUrl = urls.length === 1;
    let costLimitExceededTokenUsage = null;
    // TODO: remove the "required" fields here!! it breaks o3-mini
    if (!schema && extractOptions.options.prompt) {
        const genRes = await (0, llmExtract_1.generateSchemaFromPrompt)(extractOptions.options.prompt, logger, extractOptions.costTrackingOptions.costTracking);
    ...
    try {
        const { extract: e, warning: w, totalUsage: t, } = await (0, llmExtract_1.generateCompletions)({
    ...
    try {
        ...
        if (useAgent && extract?.shouldUseSmartscrape) {
            ...
            if (isSingleUrl) {
                smartscrapeResults = [
                    await (0, smartScrape_1.smartScrape)({
                        url: urls[0],
                        prompt: extract?.smartscrape_prompt,
                        sessionId,
                        extractId,
                ...
                smartscrapeResults = await Promise.all(pages.slice(0, 100).map(async (page) => {
                    return await (0, smartScrape_1.smartScrape)({
            ...
            const scrapedPages = smartscrapeResults.map((result) => result.scrapedPages);
            // console.log("scrapedPages", scrapedPages);
            const htmls = scrapedPages.flat().map((page) => page.html);
            // console.log("htmls", htmls);
            const markdowns = await Promise.all(htmls.map(async (html) => await (0, html_to_markdown_1.parseMarkdown)(html)));
            // console.log("markdowns", markdowns);
            extractedData = await Promise.all(markdowns.map(async (markdown) => {
                ...
                const { extract } = await (0, llmExtract_1.generateCompletions)(newExtractOptions);
                return extract;
            }));
            // console.log("markdowns", markdowns);
        ...
        if (error instanceof Error && error.message === "Cost limit exceeded") {
            costLimitExceededTokenUsage = error.cause.tokenUsage;
...
//# sourceMappingURL=extractSmartScrape.js.map