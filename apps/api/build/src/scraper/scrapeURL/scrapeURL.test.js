...
const testEnginesScreenshot = [
    undefined,
    "fire-engine;chrome-cdp",
    "fire-engine;playwright",
];
describe("Standalone scrapeURL tests", () => {
    describe.each(testEngines)("Engine %s", (forceEngine) => {
        it("Basic scrape", async () => {
            const out = await (0, _1.scrapeURL)("test:scrape-basic", "https://www.roastmywebsite.ai/", types_1.scrapeOptions.parse({}), { forceEngine, teamId: "test" }, new extraction_service_1.CostTracking());
            ...
            if (out.success) {
                ...
                expect(out.document.metadata.description).toBe("Welcome to Roast My Website, the ultimate tool for putting your website through the wringer! This repository harnesses the power of Firecrawl to scrape and capture screenshots of websites, and then unleashes the latest LLM vision models to mercilessly roast them. 🌶️");
                expect(out.document.metadata.keywords).toBe("Roast My Website,Roast,Website,GitHub,Firecrawl");
                expect(out.document.metadata.robots).toBe("follow, index");
                expect(out.document.metadata.ogTitle).toBe("Roast My Website");
                expect(out.document.metadata.ogDescription).toBe("Welcome to Roast My Website, the ultimate tool for putting your website through the wringer! This repository harnesses the power of Firecrawl to scrape and capture screenshots of websites, and then unleashes the latest LLM vision models to mercilessly roast them. 🌶️");
                expect(out.document.metadata.ogUrl).toBe("https://www.roastmywebsite.ai");
                expect(out.document.metadata.ogImage).toBe("https://www.roastmywebsite.ai/og.png");
                ...
                expect(out.document.metadata.sourceURL).toBe("https://www.roastmywebsite.ai/");
                expect(out.document.metadata.statusCode).toBe(200);
            }
        }, 30000);
        it("Scrape with formats markdown and html", async () => {
            const out = await (0, _1.scrapeURL)("test:scrape-formats-markdown-html", "https://roastmywebsite.ai", types_1.scrapeOptions.parse({
                formats: ["markdown", "html"],
            }), { forceEngine, teamId: "test" }, new extraction_service_1.CostTracking());
            // expect(out.logs.length).toBeGreaterThan(0);
            expect(out.success).toBe(true);
            if (out.success) {
                expect(out.document.warning).toBeUndefined();
        ...
        it("Scrape with onlyMainContent disabled", async () => {
            const out = await (0, _1.scrapeURL)("test:scrape-onlyMainContent-false", "https://www.scrapethissite.com/", types_1.scrapeOptions.parse({
        ...
        it("Scrape with excludeTags", async () => {
            const out = await (0, _1.scrapeURL)("test:scrape-excludeTags", "https://www.scrapethissite.com/", types_1.scrapeOptions.parse({
                onlyMainContent: false,
        ...
        it("Scrape of a page with 400 status code", async () => {
            const out = await (0, _1.scrapeURL)("test:scrape-400", "https://httpstat.us/400", types_1.scrapeOptions.parse({}), { forceEngine, teamId: "test" }, new extraction_service_1.CostTracking());
            // expect(out.logs.length).toBeGreaterThan(0);
            expect(out.success).toBe(true);
            if (out.success) {
                expect(out.document.warning).toBeUndefined();
        ...
        it("Scrape of a page with 401 status code", async () => {
            const out = await (0, _1.scrapeURL)("test:scrape-401", "https://httpstat.us/401", types_1.scrapeOptions.parse({}), { forceEngine, teamId: "test" }, new extraction_service_1.CostTracking());
            // expect(out.logs.length).toBeGreaterThan(0);
            expect(out.success).toBe(true);
            if (out.success) {
                expect(out.document.warning).toBeUndefined();
        ...
        it("Scrape of a page with 403 status code", async () => {
            const out = await (0, _1.scrapeURL)("test:scrape-403", "https://httpstat.us/403", types_1.scrapeOptions.parse({}), { forceEngine, teamId: "test" }, new extraction_service_1.CostTracking());
            // expect(out.logs.length).toBeGreaterThan(0);
            expect(out.success).toBe(true);
            if (out.success) {
                expect(out.document.warning).toBeUndefined();
        ...
        it("Scrape of a page with 404 status code", async () => {
            const out = await (0, _1.scrapeURL)("test:scrape-404", "https://httpstat.us/404", types_1.scrapeOptions.parse({}), { forceEngine, teamId: "test" }, new extraction_service_1.CostTracking());
            // expect(out.logs.length).toBeGreaterThan(0);
            expect(out.success).toBe(true);
            if (out.success) {
                expect(out.document.warning).toBeUndefined();
        ...
        it("Scrape of a page with 405 status code", async () => {
            const out = await (0, _1.scrapeURL)("test:scrape-405", "https://httpstat.us/405", types_1.scrapeOptions.parse({}), { forceEngine, teamId: "test" }, new extraction_service_1.CostTracking());
            // expect(out.logs.length).toBeGreaterThan(0);
            expect(out.success).toBe(true);
            if (out.success) {
                expect(out.document.warning).toBeUndefined();
        ...
        it("Scrape of a page with 500 status code", async () => {
            const out = await (0, _1.scrapeURL)("test:scrape-500", "https://httpstat.us/500", types_1.scrapeOptions.parse({}), { forceEngine, teamId: "test" }, new extraction_service_1.CostTracking());
            // expect(out.logs.length).toBeGreaterThan(0);
            expect(out.success).toBe(true);
            if (out.success) {
                expect(out.document.warning).toBeUndefined();
        ...
        it("Scrape a redirected page", async () => {
            const out = await (0, _1.scrapeURL)("test:scrape-redirect", "https://scrapethissite.com/", types_1.scrapeOptions.parse({}), { forceEngine, teamId: "test" }, new extraction_service_1.CostTracking());
            // expect(out.logs.length).toBeGreaterThan(0);
            expect(out.success).toBe(true);
            if (out.success) {
                expect(out.document.warning).toBeUndefined();
    ...
    describe.each(testEnginesScreenshot)("Screenshot on engine %s", (forceEngine) => {
        it("Scrape with screenshot", async () => {
            const out = await (0, _1.scrapeURL)("test:scrape-screenshot", "https://www.scrapethissite.com/", types_1.scrapeOptions.parse({
                formats: ["screenshot"],
            }), { forceEngine, teamId: "test" }, new extraction_service_1.CostTracking());
            // expect(out.logs.length).toBeGreaterThan(0);
            expect(out.success).toBe(true);
        ...
        it("Scrape with full-page screenshot", async () => {
            const out = await (0, _1.scrapeURL)("test:scrape-screenshot-fullPage", "https://www.scrapethissite.com/", types_1.scrapeOptions.parse({
    ...
    it("Scrape of a PDF file", async () => {
        const out = await (0, _1.scrapeURL)("test:scrape-pdf", "https://arxiv.org/pdf/astro-ph/9301001.pdf", types_1.scrapeOptions.parse({}), { teamId: "test" }, new extraction_service_1.CostTracking());
        // expect(out.logs.length).toBeGreaterThan(0);
        expect(out.success).toBe(true);
        if (out.success) {
            expect(out.document.warning).toBeUndefined();
    ...
    it("Scrape a DOCX file", async () => {
        const out = await (0, _1.scrapeURL)("test:scrape-docx", "https://nvca.org/wp-content/uploads/2019/06/NVCA-Model-Document-Stock-Purchase-Agreement.docx", types_1.scrapeOptions.parse({}), { teamId: "test" }, new extraction_service_1.CostTracking());
        // expect(out.logs.length).toBeGreaterThan(0);
        expect(out.success).toBe(true);
        if (out.success) {
            expect(out.document.warning).toBeUndefined();
    ...
    it("LLM extract with prompt and schema", async () => {
        const out = await (0, _1.scrapeURL)("test:llm-extract-prompt-schema", "https://firecrawl.dev", types_1.scrapeOptions.parse({
            formats: ["extract"],
            extract: {
        ...
        }), { teamId: "test" }, new extraction_service_1.CostTracking());
        // expect(out.logs.length).toBeGreaterThan(0);
        expect(out.success).toBe(true);
        if (out.success) {
            expect(out.document.warning).toBeUndefined();
    ...
    it("LLM extract with schema only", async () => {
        const out = await (0, _1.scrapeURL)("test:llm-extract-schema", "https://firecrawl.dev", types_1.scrapeOptions.parse({
            formats: ["extract"],
            extract: {
                schema: {
                    type: "object",
    ...
    test.concurrent.each(new Array(100).fill(0).map((_, i) => i))("Concurrent scrape #%i", async (i) => {
        ...
        const out = await (0, _1.scrapeURL)(id, url, types_1.scrapeOptions.parse({}), { teamId: "test" }, new extraction_service_1.CostTracking());
        ...
        if (!out.success)
            console.error(JSON.stringify(out, replacer));
        expect(out.success).toBe(true);
        if (out.success) {
            expect(out.document.warning).toBeUndefined();
            expect(out.document).toHaveProperty("markdown");
...
//# sourceMappingURL=scrapeURL.test.js.map