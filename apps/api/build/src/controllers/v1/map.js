...
async function queryIndex(url, limit, useIndex, includeSubdomains) {
    ...
    if (urlSplits.length === 1) {
        ...
        const domainLinks = includeSubdomains ? await (0, index_1.queryIndexAtDomainSplitLevel)(hostname, limit, 14 * 24 * 60 * 60 * 1000) : [];
        const splitLinks = await (0, index_1.queryIndexAtSplitLevel)(url, limit, 14 * 24 * 60 * 60 * 1000);
        return Array.from(new Set([...domainLinks, ...splitLinks]));
    }
    else {
        return await (0, index_1.queryIndexAtSplitLevel)(url, limit);
    }
}
async function getMapResults({ url, search, limit = MAX_MAP_LIMIT, ignoreSitemap = false, includeSubdomains = true, crawlerOptions = {}, teamId, origin, includeMetadata = false, allowExternalLinks, abort = new AbortController().signal, // noop
mock, filterByPath = true, flags, useIndex = true, timeout, }) {
    ...
    if (crawlerOptions.sitemapOnly) {
        ...
        }, true, true, timeout ?? 30000, abort, mock);
        if (sitemap > 0) {
            links = links
                .slice(1)
                .map((x) => {
        ...
        if (!ignoreSitemap) {
            try {
                await crawler.tryGetSitemap((urls) => {
                    links.push(...urls);
                }, true, false, timeout ?? 30000, abort);
            }
            catch (e) {
                logger_1.logger.warn("tryGetSitemap threw an error", { error: e });
            }
        }
        if (!cachedResult) {
        ...
        if (filterByPath && !allowExternalLinks) {
            try {
                ...
                logger_1.logger.warn(`Failed to parse URL for path filtering: ${url}`, { error: e });
            }
        }
...
async function mapController(req, res) {
    ...
    logger_1.logger.info("Map request", {
        request: req.body,
        originalRequest,
        ...
        result = await Promise.race([
            ...
            ...(req.body.timeout !== undefined ? [
                new Promise((resolve, reject) => setTimeout(() => {
                    abort.abort(new types_1.TimeoutSignal());
                    reject(new types_1.TimeoutSignal());
                }, req.body.timeout))
            ] : []),
        ]);
    }
    catch (error) {
        if (error instanceof types_1.TimeoutSignal || error === "timeout") {
