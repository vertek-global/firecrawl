...
exports.CostTracking = exports.CostLimitExceededError = void 0;
exports.performExtraction = performExtraction;
const types_1 = require("../../controllers/v1/types");
const logger_1 = require("../logger");
const url_processor_1 = require("./url-processor");
const document_scraper_1 = require("./document-scraper");
...
const dereference_schema_1 = require("./helpers/dereference-schema");
const spread_schemas_1 = require("./helpers/spread-schemas");
const transform_array_to_obj_1 = require("./helpers/transform-array-to-obj");
const mix_schema_objs_1 = require("./helpers/mix-schema-objs");
const ajv_1 = __importDefault(require("ajv"));
const ajv = new ajv_1.default();
const extract_redis_1 = require("./extract-redis");
const deduplicate_objs_array_1 = require("./helpers/deduplicate-objs-array");
const merge_null_val_objs_1 = require("./helpers/merge-null-val-objs");
const config_1 = require("./config");
const llm_cost_1 = require("./usage/llm-cost");
const analyzeSchemaAndPrompt_1 = require("./completions/analyzeSchemaAndPrompt");
const batchExtract_1 = require("./completions/batchExtract");
const singleAnswer_1 = require("./completions/singleAnswer");
const source_tracker_1 = require("./helpers/source-tracker");
...
const nanProof = (n) => isNaN(n ?? 0) ? 0 : (n ?? 0);
class CostTracking {
    calls = [];
    limit = null;
...
async function performExtraction(extractId, options) {
    ...
    try {
        ...
        if ((!request.urls || request.urls.length === 0) && request.prompt) {
            ...
            const rephrasedPrompt = await (0, url_processor_1.generateBasicCompletion)((0, build_prompts_1.buildRephraseToSerpPrompt)(request.prompt), costTracking);
        ...
        if (request.urls && request.urls.length === 0) {
            ...
            const tokens_billed = 300 + (0, llm_cost_1.calculateThinkingCost)(costTracking);
            (0, log_job_1.logJob)({
                job_id: extractId,
                success: false,
            ...
            await (0, credit_billing_1.billTeam)(teamId, subId, tokens_billed, logger, true).catch((error) => {
                logger.error(`Failed to bill team ${teamId} for thinking tokens: ${error}`);
            });
            return {
                success: false,
        ...
        if (request.__experimental_cacheMode == "load" &&
        ...
        if (!reqSchema && request.prompt) {
            const schemaGenRes = await (0, llmExtract_1.generateSchemaFromPrompt)(request.prompt, logger, costTracking);
        ...
        if (reqSchema) {
            reqSchema = await (0, dereference_schema_1.dereferenceSchema)(reqSchema);
        }
        ...
        const { isMultiEntity, multiEntityKeys, reasoning, keyIndicators, tokenUsage: schemaAnalysisTokenUsage, } = await (0, analyzeSchemaAndPrompt_1.analyzeSchemaAndPrompt)(urls, reqSchema, request.prompt ?? "", logger, costTracking);
        ...
        const urlPromises = urls.map((url) => (0, url_processor_1.processUrl)({
            url,
            prompt: request.prompt,
            teamId,
        ...
        }, logger.child({ module: "extract", method: "processUrl", url }), costTracking, acuc?.flags ?? null));
        ...
        if (isMultiEntity && reqSchema) {
            ...
            const { singleAnswerSchema, multiEntitySchema } = await (0, spread_schemas_1.spreadSchemas)(reqSchema, multiEntityKeys);
            ...
            const scrapePromises = links.map((url) => {
                if (!docsMap.has((0, canonical_url_1.normalizeUrl)(url))) {
                    return (0, document_scraper_1.scrapeDocument)({
                        url,
                        teamId,
                        origin: "extract",
                        timeout,
            ...
            await (0, extract_redis_1.updateExtract)(extractId, {
                ...
                steps: [
                    {
                        step: extract_redis_1.ExtractStep.MULTI_ENTITY_AGENT_SCRAPE,
                        startedAt: Date.now(),
                        finishedAt: null
                    },
                ],
            ...
            for (let i = 0; i < chunks.length; i++) {
                ...
                const chunkPromises = chunk.map(async (doc) => {
                    try {
                        ...
                        const completionPromise = (0, batchExtract_1.batchExtractPromise)({
                            multiEntitySchema,
                            links,
                            prompt: request.prompt ?? "",
                            systemPrompt: request.systemPrompt ?? "",
                            doc,
                            useAgent: (0, types_1.isAgentExtractModelValid)(request.agent?.model),
                            extractId,
                            sessionId,
                            costTracking,
                        }, logger);
                        // Race between timeout and completion
                        const multiEntityCompletion = (await completionPromise);
            ...
            try {
                ...
                const sourceTracker = new source_tracker_1.SourceTracker();
                ...
                try {
                    multiEntityResult = (0, transform_array_to_obj_1.transformArrayToObject)(multiEntitySchema, multiEntityCompletions);
                ...
                try {
                    multiEntityResult = (0, deduplicate_objs_array_1.deduplicateObjectsArray)(multiEntityResult);
                ...
                try {
                    multiEntityResult = (0, merge_null_val_objs_1.mergeNullValObjs)(multiEntityResult);
        ...
        if (rSchema &&
            ...
            let { extract: completionResult, tokenUsage: singleAnswerTokenUsage, sources: singleAnswerSources, } = await (0, singleAnswer_1.singleAnswerCompletion)({
                singleAnswerDocs,
                rSchema,
                links,
                prompt: request.prompt ?? "",
            ...
            singleAnswerResult = (0, transform_array_to_obj_1.transformArrayToObject)(rSchema, completionResult);
            singleAnswerResult = (0, deduplicate_objs_array_1.deduplicateObjectsArray)(singleAnswerResult);
                ...
                if (rSchema?.properties) {
                    Object.keys(rSchema.properties).forEach((key) => {
                        if (completionResult[key] !== undefined) {
                            sources[key] =
                                singleAnswerSources ||
                                    singleAnswerDocs.map((doc) => doc.metadata.url || doc.metadata.sourceURL || "");
                        }
                    });
                }
            }
        ...
        let finalResult = reqSchema
            ? await (0, mix_schema_objs_1.mixSchemaObjects)(reqSchema, singleAnswerResult, multiEntityResult, logger.child({ method: "mixSchemaObjects" }))
        ...
        const llmUsage = (0, llm_cost_1.estimateTotalCost)(tokenUsage);
        let tokensToBill = (0, llm_cost_1.calculateFinalResultCost)(finalResult) + (0, llm_cost_1.calculateThinkingCost)(costTracking);
        if (config_1.CUSTOM_U_TEAMS.includes(teamId)) {
            tokensToBill = 1;
        }
        // Bill team for usage
        await (0, credit_billing_1.billTeam)(teamId, subId, tokensToBill, logger, true).catch((error) => {
        ...
        logger.debug("Done!");
        if (request.__experimental_cacheMode == "save" &&
            request.__experimental_cacheKey) {
