...
exports.rerankLinks = rerankLinks;
exports.rerankLinksWithLLM = rerankLinksWithLLM;
...
async function rerankLinks(mappedLinks, searchQuery, urlTraces, flags) {
    ...
    const linksAndScores = await (0, ranker_1.performRanking)(mappedLinksRerank, mappedLinks.map((l) => l.url), searchQuery);
    // First try with high threshold
    let filteredLinks = filterAndProcessLinks(mappedLinks, linksAndScores, config_1.extractConfig.RERANKING.INITIAL_SCORE_THRESHOLD_FOR_RELEVANCE, flags);
    ...
    if (filteredLinks.length < config_1.extractConfig.RERANKING.MIN_REQUIRED_LINKS) {
        ...
        filteredLinks = filterAndProcessLinks(mappedLinks, linksAndScores, config_1.extractConfig.RERANKING.FALLBACK_SCORE_THRESHOLD_FOR_RELEVANCE, flags);
...
function filterAndProcessLinks(mappedLinks, linksAndScores, threshold, flags) {
    return linksAndScores
        .filter((x) => x.score > threshold)
        .map((x) => mappedLinks.find((link) => link.url === x.link))
        .filter((x) => x !== undefined && x.url !== undefined && !(0, blocklist_1.isUrlBlocked)(x.url, flags));
}
async function rerankLinksWithLLM(options) {
            ...
            relevantLinks: {
                ...
                items: {
                    ...
                    properties: {
                        ...
                        reason: {
                            type: "string",
                            description: "The reason why you chose the score for this link given the intent.",
                        },
                    },
                ...
                try {
                    const completionPromise = (0, llmExtract_1.generateCompletions)({
                        ...
                        options: {
                            ...
                            prompt: (0, build_prompts_1.buildRerankerUserPrompt)(searchQuery),
                            schema: schema,
