...
const html_to_markdown_1 = require("../html-to-markdown");
describe("parseMarkdown", () => {
    it("should correctly convert simple HTML to Markdown", async () => {
        const html = "<p>Hello, world!</p>";
        const expectedMarkdown = "Hello, world!";
        await expect((0, html_to_markdown_1.parseMarkdown)(html)).resolves.toBe(expectedMarkdown);
    });
    ...
    it("should handle various types of invalid HTML gracefully", async () => {
        ...
        for (const { html, expected } of invalidHtmls) {
            await expect((0, html_to_markdown_1.parseMarkdown)(html)).resolves.toBe(expected);
        }
    });
});
//# sourceMappingURL=html-to-markdown.test.js.map