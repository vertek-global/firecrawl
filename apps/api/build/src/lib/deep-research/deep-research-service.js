...
async function performDeepResearch(options) {
    ...
    const state = new research_manager_1.ResearchStateManager(researchId, teamId, options.maxDepth, logger, options.query);
    const llmService = new research_manager_1.ResearchLLMService(logger);
    ...
    try {
        while (!state.hasReachedMaxDepth() && urlsAnalyzed < maxUrls) {
            ...
            const searchQueries = (await llmService.generateSearchQueries(nextSearchTopic, state.getFindings(), costTracking)).slice(0, 3);
            ...
            const searchPromises = searchQueries.map(async (searchQuery) => {
                const response = await (0, search_1.searchAndScrapeSearchResult)(searchQuery.query, {
            ...
            logger.debug("[Deep Research] Search results count:", { count: searchResults.length });
            ...
            logger.debug("[Deep Research] New unique results count:", { length: newSearchResults.length });
            ...
            await state.addFindings(newSearchResults.map((result) => ({
            ...
            const analysis = await llmService.analyzeAndPlan(state.getFindings(), currentTopic, timeRemaining, options.systemPrompt ?? "", costTracking);
            if (checkTimeLimit()) {
        ...
        if (options.formats.includes('json')) {
            finalAnalysisJson = await llmService.generateFinalAnalysis(options.query, state.getFindings(), state.getSummaries(), options.analysisPrompt, costTracking, options.formats, options.jsonOptions);
        }
        if (options.formats.includes('markdown')) {
            finalAnalysis = await llmService.generateFinalAnalysis(options.query, state.getFindings(), state.getSummaries(), options.analysisPrompt, costTracking);
        }
        await state.addActivity([{
                type: "synthesis",
                status: "complete",
        ...
        await (0, deep_research_redis_1.updateDeepResearch)(researchId, {
            status: "completed",
            finalAnalysis: finalAnalysis,
            json: finalAnalysisJson,
        });
        // Bill team for usage based on URLs analyzed
        (0, credit_billing_1.billTeam)(teamId, subId, credits_billed, logger).catch((error) => {
            logger.error(`Failed to bill team ${teamId} for ${urlsAnalyzed} URLs analyzed`, { teamId, count: urlsAnalyzed, error });
        });
        return {
            success: true,
            data: {
